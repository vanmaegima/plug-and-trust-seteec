...
int send(int s, char *buf, int len, int flags) {
    ...
    int ret = (int)qcom_send(s, sendBuf, len, 0); // FIXME: Options
...
exit:
    ...
    addr.sin_port = atoi(server_port);

    uint32_t s_addr = 0;
    status = qcom_dnsc_get_host_by_name((char *)hostname, &s_addr);
    addr.sin_addr.s_addr = s_addr;

    if (status != A_OK || addr.sin_addr.s_addr == 0) {
        EXIT_STATUS_MSG(IOT_AGENT_FAILURE, "qcom_dnsc_get_host_by_name failed");
    }

    printf("Connecting to to %s %d\n", hostname, addr.sin_port);

    *pSocket_id = qcom_socket(ATH_AF_INET, SOCK_STREAM_TYPE, 0);
    ASSERT_OR_EXIT_MSG(*pSocket_id >= 0, "Error in opening socket\n");

    struct sockaddr saddr;
    memcpy(&saddr, &addr, sizeof(struct sockaddr));
    status = (A_STATUS)qcom_connect(*pSocket_id, &saddr, sizeof(addr));

    ASSERT_OR_EXIT_MSG(status == A_OK, "qcom_connect failed");
    IOT_AGENT_INFO("Connection to provisioning server established!");
...
iot_agent_status_t process_result(remote_provisioning_TLV_t* resultTlv, size_t* num_objects)
{
    ...
    *num_objects = read_id_list_bytes / sizeof(*num_objects);
