...
iot_agent_status_t iot_agent_claimcode_inject(ex_sss_boot_ctx_t *pCtx, char *claimcode, size_t claimcode_len)
{
    ...
    if (sss_key_object_get_handle(&obj, EDGELOCK2GO_USER_ID_DEMO) == (sss_status_t)kStatus_SSS_Success)
    {
        edgelock2go_auth_id = EDGELOCK2GO_USER_ID_DEMO;
    }
    else  if (sss_key_object_get_handle(&obj, EDGELOCK2GO_USER_ID) == (sss_status_t)kStatus_SSS_Success)
    {
    ...
    sss_status = sss_key_object_get_handle(&obj, CLAIMCODE_OBJ_ID);
