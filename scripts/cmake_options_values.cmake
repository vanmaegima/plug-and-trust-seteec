...
IF(NXPInternal)
    SET_PROPERTY(
        ...
        PROPERTY
            STRINGS
            "None;A71CH;A71CL;A71CH_SIM;SE05X_A;SE05X_B;SE05X_C;SE05X_L;SE051_UWB;AUTH;LoopBack;"
    )
ELSE()
    SET_PROPERTY(
        CACHE Applet
        PROPERTY
            STRINGS
            "None;A71CH;SE05X_A;SE05X_B;SE05X_C;AUTH;"
    )
ENDIF()
...
IF("${Applet}" STREQUAL "SE051_UWB")
    SET(WithApplet_SE051_UWB ON)
    SET(SSS_HAVE_APPLET_SE051_UWB "1")
ELSE()
    SET(WithApplet_SE051_UWB OFF)
    SET(SSS_HAVE_APPLET_SE051_UWB "0")
ENDIF()

IF("${Applet}" STREQUAL "AUTH")
    SET(WithApplet_AUTH ON)
    SET(SSS_HAVE_APPLET_AUTH "1")
ELSE()
    SET(WithApplet_AUTH OFF)
...
ELSEIF("${Applet}" STREQUAL "SE05X_L")
    # OK
ELSEIF("${Applet}" STREQUAL "SE051_UWB")
    # OK
ELSEIF("${Applet}" STREQUAL "AUTH")
    # OK
ELSEIF("${Applet}" STREQUAL "LoopBack")
    # OK
ELSE()
    MESSAGE(SEND_ERROR "For 'Applet' '${Applet}' is invalid.")
    MESSAGE(STATUS "Only supported values are 'None, A71CH, A71CL, A71CH_SIM, SE05X_A, SE05X_B, SE05X_C, SE05X_L, SE051_UWB, AUTH, LoopBack'")
ENDIF()
...
IF("${SMCOM}" STREQUAL "JRCP_V1_AM")
    SET(WithSMCOM_JRCP_V1_AM ON)
    SET(SSS_HAVE_SMCOM_JRCP_V1_AM "1")
ELSE()
    SET(WithSMCOM_JRCP_V1_AM OFF)
...
ELSEIF("${SMCOM}" STREQUAL "JRCP_V1")
    # OK
ELSEIF("${SMCOM}" STREQUAL "JRCP_V1_AM")
    # OK
...
IF("${SBL}" STREQUAL "None")
    SET(WithSBL_None ON)
    SET(SSS_HAVE_SBL_NONE "1")
ELSE()
    SET(WithSBL_None OFF)
    SET(SSS_HAVE_SBL_NONE "0")
ENDIF()

IF("${SBL}" STREQUAL "SBL_LPC55S")
    SET(WithSBL_SBL_LPC55S ON)
    SET(SSS_HAVE_SBL_SBL_LPC55S "1")
ELSE()
    SET(WithSBL_SBL_LPC55S OFF)
    SET(SSS_HAVE_SBL_SBL_LPC55S "0")
ENDIF()

IF("${SBL}" STREQUAL "None")
    # OK
ELSEIF("${SBL}" STREQUAL "SBL_LPC55S")
    # OK
ELSE()
    MESSAGE(SEND_ERROR "For 'SBL' '${SBL}' is invalid.")
